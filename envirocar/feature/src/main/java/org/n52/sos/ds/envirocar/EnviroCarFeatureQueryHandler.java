...
public class EnviroCarFeatureQueryHandler implements FeatureQueryHandler {
    ...
    public SosEnvelope getEnvelopeForFeatureIDs(Collection<String> featureIDs, Object connection)
            throws OwsExceptionReport {
        TrackDao trackDAO = EnviroCarDaoFactoryHolder.getEnviroCarDaoFactory(connection).getTrackDAO();
        Envelope bBox = trackDAO.getBBox(new TrackFilter().setIdentifier(featureIDs));
        Geometry geometry = new GeometryFactory().toGeometry(bBox);
        geometry.setSRID(getDefaultEPSG());
        Geometry geom = GeometryHandler.getInstance().switchCoordinateAxisOrderIfNeeded(geometry);
